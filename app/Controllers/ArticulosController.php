...
class ArticulosController extends BaseController
{
    ...
    public function index(Request $request): Response
    {
        ...
        if (!empty($search)) {
            ...
            $whereClause = "MATCH(ca.origen, ca.producto, ca.marca, ca.modelo, ca.numero_serie) AGAINST (:search IN BOOLEAN MODE)";
            $params['search'] = $booleanSearch;
        }

        $db = new QueryBuilder();
        $response = $db->paginate(
            table: 'ca_articulo ca',
            page: $page,
            perPage: $limit,
            columns: [
                'ca.uuid',
                ...
                'ca.modelo',
                'ca.numero_serie',
                'DATE_FORMAT(ca.fecha_alta, "%d/%m/%Y %T") fecha_alta',
                'DATE_FORMAT(ca.fecha_mod, "%d/%m/%Y %T") fecha_mod',
            ],
            where: $whereClause,
            params: $params,
            extras: 'ORDER BY ca.fecha_alta DESC'
        );

        if (!$response->success) {
            return $this->error($response->error ?? 'Error al obtener los articulos.', 500);
        }
